 ...
 * 历史注记：lifecycle/comm-health 两通道随执行引擎退役删除（2026-08-28），
 * 本测试同步收窄到 error/app 两文件通道 + console/broadcast 直挂。
 */
public class LogbackConfigLoadTest {

    private static final long ONE_GB = 1L << 30;

    /** 通道表：异步 appender 名 -> 通道文件名（诊断 DEBUG 是结构缝，不落配置） */
    ...
    @Test
    public void twoFileChannelsConfiguredWithoutErrorStatus() {
    ...
    public void asyncParametersAndTotalSizeCapBudget() {
        ...
        // 保留期（天）走 policy getter：错误 90 / 应用 30
        int[] expectedHistory = {90, 30};
        ...
        // 预算兜底：两通道 cap 之和必须留出 1GB 上限的余量（诊断通道是结构缝，不落配置）
        ...
        assertEquals("应有且仅有两个通道配置 totalSizeCap", CHANNELS.length, capCount);
