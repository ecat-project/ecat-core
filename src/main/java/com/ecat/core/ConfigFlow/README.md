...
### 原则 1.5：复用 comm schema 时用 Builder 覆盖默认值（勿 copy-paste）

`SerialCommConfigSchema` / `TcpClientCommConfigSchema` / `TcpServerCommConfigSchema` /
`ModbusTcpCommConfigSchema` / `ModbusRtuCommConfigSchema` / `ModbusCommTypeSchema` 均提供 `Builder`。
当你的设备需要覆盖某个 comm 字段的默认值（如本设备 serial 超时要 2000ms、或 TCP host 必填无默认、port 默认 7030）时，
**用 Builder 覆盖**，不要把整段 schema 复制粘贴到自己集成里改（复制后会与主 schema 演进脱节）。

```java
// ✅ 推荐：Builder 只覆盖关心的字段，其余继承标准默认
.addField(new SchemaConfigItem("comm", true,
    TcpClientCommConfigSchema.builder()
        .host(null)        // null = 去掉预填默认,字段仍必填
        .port(7030)
        .readTimeout(60)
        .build().createSchema()))

// ❌ 避免：为改 host/port 把整个 TcpClientCommConfigSchema 抄到本集成重写
//        （主 schema 加字段/改 range 后这里不会同步）

// 不需要覆盖任何默认值时，旧写法仍可用（向后兼容）：
new TcpClientCommConfigSchema().createSchema()   // 等价于 builder().build().createSchema()
```

`ModbusRtuCommConfigSchema` 嵌套 serial schema，要覆盖被嵌套的 serial 默认（如超时），用 `serialSchema(...)` 注入：
