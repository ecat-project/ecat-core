...
/**
 * 轮询事务因「源锁忙」被放弃的信号（L2 传输 SDK 层共用词汇——17 号 §1：modbus→serial
 * 的 L2-L2 横向共用件批 0 下沉 core；S4 归位 Task 词汇包——纯异常词汇，与调度引擎无关，
 * 不随 engine/execution 消亡）。
 *
 * <p><b>语义</b>：只由<b>轮询入口</b>（executePolling 的 {@code acquirePollingBounded}）产生——
 * 锁忙时先并入 FIFO 有界等待（等待发生在 IO 旁池线程，调用线程零 park），预算耗尽才以本
 * 异常放弃本周期、下周期再试。周期任务的 whenComplete 消费方应把本异常
